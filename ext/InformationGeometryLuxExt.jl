...
function NeuralNet(In::Int, Out::Int, N::Int=2, hidden::Int=1; positive::Bool=false, activation::Function=tanh, HiddenActivation::Function=activation, 
                            FinalActivation::Function=(positive ? softplus : identity), gain::Real=1, kwargs...)
    Lux.Chain(Lux.Dense(In, N, HiddenActivation; init_weight=Lux.kaiming_uniform(; gain), kwargs...), 
            [Lux.Dense(N, N, HiddenActivation; init_weight=Lux.kaiming_uniform(; gain), kwargs...) for i in 1:hidden]..., 
            Lux.Dense(N, Out, FinalActivation; init_weight=Lux.kaiming_uniform(; gain), kwargs...))
end
...
function NormalizedNeuralModel(xd::Int, yd::Int, args...; rng=Random.default_rng(), 
    ...
    ModelMap((yd == 1 ? (x,p)->M(x,p)[1] : M), nothing, nothing, (xd, yd, length(p_NN)); startp=p_NN), p_NN, U
end
