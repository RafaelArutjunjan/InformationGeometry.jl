...
@test sum(abs, MLE(R)-MLE(R2)) < 3e-8
@test sum(abs, MLE(R)-MLE(R3)) < 3e-8
@test sum(abs, MLE(LocalMultistartFit(DM, MLE(R); N=5)) -MLE(R)) < 3e-8

@test sum(abs, IncrementalTimeSeriesFit(DM, MLE(R))-MLE(R)) < 3e-8
...
@test sum(abs, MLE(RU)-MLE(RU2)) < 3e-8
@test sum(abs, MLE(RU)-MLE(RU3)) < 3e-8
