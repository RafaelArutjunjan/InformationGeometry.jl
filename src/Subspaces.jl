...
function PlanarDataModel(DM::AbstractDataModel, PL::Plane, mle::AbstractVector{<:Number}=DecomposeWRTPlane(PL, ProjectOntoPlane(PL, MLE(DM))))
    ...
    model = Predictor(DM);      dmodel = dPredictor(DM);    Proj = Projector(PL)
    ModelParamGetter = PlaneCoordinates(PL)∘GetOnlyModelParams(DM)
    newmod = (x,θ::AbstractVector{<:Number}; kwargs...) -> model(x, ModelParamGetter(θ); kwargs...)
    dnewmod = (x,θ::AbstractVector{<:Number}; kwargs...) -> dmodel(x, ModelParamGetter(θ); kwargs...) * Proj
...
function ConstructCube(sols::AbstractVector{<:AbstractODESolution}, Npoints::Int=300; N::Int=Npoints, kwargs...)
    mapreduce(sol->ConstructCube(sol; N, kwargs...), Base.union, sols)
end
...
function ConstructCube(Planes::AbstractVector{<:Plane}, sols::AbstractVector{<:AbstractODESolution}; N::Int=300, kwargs...)
    ...
    reduce(Base.union, map((x,y)->ConstructCube(x,y; N=N, kwargs...), Planes, sols))
end
...
Base.vcat(C1::HyperCube, C2::HyperCube, args::HyperCube...) = vcat(vcat(C1, C2), args...)
...
Base.getindex(C::HyperCube, inds::AbstractVector{<:Union{<:Int,<:Bool}}) = @views HyperCube(C.L[inds], C.U[inds])
