...
function ParamSummary(DM::AbstractDataModel, mle::AbstractVector{<:Number}=MLE(DM); FisherFn::Function=FisherMetric(DM), BoundaryThreshold::Real=1/200, display_size=(-1,-1), 
                                        pnames::AbstractVector{<:AbstractString}=pnames(DM), kwargs...) # vertical_crop_mode=:middle
    ...
    Mleuncert = round.(MLEuncert(DM, mle, FisherFn(mle);verbose=false); sigdigits=15)
    ...
    if !isnothing(IsLin) && any(IsLin)
        PrettyTable([1:length(pnames) pnames L Mleuncert U IsLin]; column_labels=["i", "Parameter", "Lower Bound", "MLE", "Upper Bound", "Linear Dependence"], alignment=[:c, :l, :c, :c, :c, :c], highlighters=H, display_size, kwargs...)
    else
        PrettyTable([1:length(pnames) pnames L Mleuncert U]; column_labels=["i", "Parameter", "Lower Bound", "MLE", "Upper Bound"], alignment=[:c, :l, :c, :c, :c], highlighters=H, display_size, kwargs...)
    end
end
...
function Base.show(io::IO, ::MIME"text/plain", CG::ConditionGrid)
    ...
    isnothing(LogPr) || println(io, "Log prior at MLE: "*string(round(LogPr; sigdigits=5)))
    S = try ParamTrafoString(CG) catch; nothing end
    !isnothing(S) && print(io, "Parameter Transformations: " * S)
end
# Single line display
...
function Base.show(io::IO, ::MIME"text/plain", P::ParamTrafo)
    ...
    S = try ParamTrafoString(P) catch; nothing end
    !isnothing(S) && print(io, "Parameter Transformations: " * S)
end
# Single line display
