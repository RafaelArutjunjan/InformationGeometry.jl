...
### Computes uncertainties based on model predictions instead of data
...
function ysigmaProper(DM::AbstractDataModel, mle::AbstractVector=MLE(DM); ParamGetter::Function=GetOnlyModelParams(DM), kwargs...)
    !HasEstimatedUncertainties(DM) && return ysigma(DM, mle; kwargs...)
    woundX = WoundX(DM);    woundY = Windup(EmbeddingMap(Val(true), Predictor(DM), ParamGetter(mle), woundX), ydim(DM))
...
function yInvCovProper(DM::AbstractDataModel, mle::AbstractVector=MLE(DM); ParamGetter::Function=GetOnlyModelParams(DM), kwargs...)
