...
function MultistartFit(DM::AbstractDataModel, InitialPointGen::Union{AbstractVector{<:AbstractVector{<:Number}}, Distributions.MultivariateDistribution, Base.Generator, SOBOL.AbstractSobolSeq}; 
                                        ...
                                        meth=((isnothing(LogPriorFn) && DM isa DataModel && !HasEstimatedUncertainties(DM) && isloaded(:LsqFit)) ? nothing : DefaultFirstOrderOptimizer), kwargs...)
...
function MultistartFit(costfunction::Function, InitialPointGen::Union{AbstractVector{<:AbstractVector{<:Number}}, Distributions.MultivariateDistribution, Base.Generator, SOBOL.AbstractSobolSeq}; showprogress::Bool=true, N::Int=100, maxval::Real=1e3, plot::Bool=false, 
                                        ...
                                        TransformSample::Function=identity, LogLikelihoodFn::Function=Negate(CostFunction), TransformFinalPoints::Function=identity,
...
"""
LocalMultistartFit(DM::AbstractDataModel, Confnum::Real=2; MLE::AbstractVector=MLE(DM), kwargs...) = LocalMultistartFit(DM, MLE, Confnum; kwargs...)
LocalMultistartFit(DM::AbstractDataModel, mle::AbstractVector{<:Number}, Confnum::Real=2; Fisher::AbstractMatrix=FisherMetric(DM, mle), kwargs...) = MultistartFit(DM; MultistartDomain=GetProfileDomainCube(DM, mle, Confnum; Fisher), kwargs...)
...
"""
   StochasticProfileLikelihood(DM::AbstractDataModel, C::HyperCube=Domain(DM); TargetTime=30, Nsingle::Int=5, N::Int=Nsingle^length(C), nbins::Int=Nsingle, parallel::Bool=true, maxval::Real=1e5, BiLog::Bool=true, kwargs...)
...
function StochasticProfileLikelihood(DM::AbstractDataModel, C::HyperCube=GetDomainSafe(DM); TargetTime::Real=30, Nsingle::Union{Nothing,Int}=nothing, parallel::Bool=true, N::Int=isnothing(Nsingle) ? GetNFromTargetTime(DM, TargetTime; parallel) : Nsingle^length(C), 
                                                factor::Real=1, nbins::Int=isnothing(Nsingle) ? Int(ceil(clamp(factor*(N^(1/length(C))),3,100))) : Nsingle, maxval::Real=1e5, Domain::HyperCube=C∩FullDomain(length(C),maxval), TransformSample::Function=identity, seed::Int=rand(0:Int(1e7)), kwargs...)
...
function StochasticProfileLikelihood(df::AbstractDataFrame, LikelihoodCol::Int=size(df,2); pnames::AbstractVector{<:AbstractString}=DataFrames.names(df)[Not(LikelihoodCol)], kwargs...)
    StochasticProfileLikelihood([Vector(x) for x in eachrow(df[:,Not(LikelihoodCol)])], Vector(@view df[:,LikelihoodCol]); pnames, kwargs...)
end
...
function _GetStochasticProfile(Points::AbstractVector{<:AbstractVector}, Likelihoods::AbstractVector{<:Number}, ind::Int; factor::Real=1, nbins::Int=Int(ceil(clamp(factor*(length(Likelihoods)^(1/length(Points[1]))),3,100))), Extremizer::Function=findmax, 
                                    UseSorted::Bool=true, pval::AbstractVector=getindex.(Points,ind), pBins::AbstractVector=collect(HistoBins(pval,nbins)), OffsetResults::Bool=true, OffsetResultsBy::Union{Nothing,Number}=nothing, pnames=String[])
    ...
    OffsetResults && (isnothing(OffsetResultsBy) ? (Res .-= Extremizer(Res)[1]) : (Res .-= OffsetResultsBy))
...
function _GetStochastic2DProfile(Points::AbstractVector{<:AbstractVector}, Likelihoods::AbstractVector{<:Number}, idxs::AbstractVector{<:Int}; Extremizer::Function=findmax, OffsetResults::Bool=true, OffsetResultsBy::Union{Nothing,Number}=nothing, 
                            factor::Real=1, nxbins::Int=Int(ceil(clamp(factor*(length(Likelihoods)^(1/length(Points[1]))),3,100))), nybins::Int=Int(ceil(clamp(factor*(length(Likelihoods)^(1/length(Points[1]))),3,100))), 
...
# Collective
function StochasticProfileLikelihoodPlot(Points::AbstractVector{<:AbstractVector}, Likelihoods::AbstractVector{<:Number}; CutoffVal::Real=Inf, Inds::AbstractVector{<:Int}=1:findlast(x->isfinite(x) && abs(x)<CutoffVal,Likelihoods), factor::Real=1, nbins::Int=Int(ceil(clamp(factor*(length(Inds)^(1/length(Points[1]))),3,100))), BiLog::Bool=true, Trafo::Function=(BiLog ? InformationGeometry.BiLog : identity), Extremizer::Function=findmax,
                                pnames::AbstractVector{<:AbstractString}=CreateSymbolNames(length(Points[1])), legend=false, OffsetResults::Bool=false, OffsetResultsBy::Union{Nothing,Number}=nothing, Cutoff::Int=400000, MLE::Union{Nothing,AbstractVector{<:Number}}=nothing, size=PlotSizer(length(pnames)), kwargs...)
    P = [StochasticProfileLikelihoodPlot((@view Points[Inds]), (@view Likelihoods[Inds]), i; nbins, Trafo, Extremizer, xlabel=string(pnames[i]), legend, OffsetResults, OffsetResultsBy, MLE) for i in eachindex(pnames)]
    Offset = OffsetResults ? (isnothing(OffsetResultsBy) ? Likelihoods[1] : OffsetResultsBy) : 0.0
    AddedPlots = []
...
function StochasticProfileLikelihoodPlot(Points::AbstractVector{<:AbstractVector}, Likelihoods::AbstractVector{<:Number}, ind::Int; nbins::Int=Int(ceil(clamp(0.5*(length(Likelihoods)^(1/length(Points[1]))),3,100))), Extremizer::Function=findmax, 
                                 BiLog::Bool=true, Trafo::Function=(BiLog ? InformationGeometry.BiLog : identity), pval::AbstractVector=getindex.(Points, ind), pBins::AbstractVector=collect(HistoBins(pval, nbins)), OffsetResults::Bool=false, OffsetResultsBy::Union{Nothing,Number}=nothing, 
                                 xlabel="Parameter $ind", MLE::Union{Nothing,AbstractVector{<:Number}}=nothing, kwargs...)
    _, Res, ResPoints = _GetStochasticProfile(Points, Likelihoods, ind; nbins, Extremizer, pval, pBins, OffsetResults, OffsetResultsBy)
    res = Trafo.(Res)
    ...
    !isnothing(MLE) && length(MLE) == length(Points[1]) && RecipesBase.plot!(Plt, [MLE[ind]]; st=:vline, line=:dash, c=:green, lw=2, label="MLE")
    Plt
end
...
@recipe function f(Points::AbstractVector{<:AbstractVector}, Likelihoods::AbstractVector{<:Number}, ind::Int, ::StochasticProfileVal)
    ...
    OffsetResultsBy = get(plotattributes, :OffsetResultsBy, nothing)
    pnames = get(plotattributes, :pnames, CreateSymbolNames(length(Points[1])))
    pval = get(plotattributes, :pval, getindex.(Points, ind))
    pBins = get(plotattributes, :pBins, collect(HistoBins(pval, Nbins)))

    _, Res, ResPoints = _GetStochasticProfile(Points, Likelihoods, ind; nbins=Nbins, Extremizer, pval=getindex.(Points, ind), pBins, OffsetResults, OffsetResultsBy)
    res = Trafo.(Res)
    HitsPerBin = float.(CountInBin(pBins, Points, ind));  HitsPerBin ./= maximum(HitsPerBin)
...
@recipe function f(Points::AbstractVector{<:AbstractVector}, Likelihoods::AbstractVector{<:Number}, idxs::AbstractVector{<:Int}, ::StochasticProfile2DVal;
                ...
                Extremizer = findmax, BiLog = true, Trafo = (BiLog ? InformationGeometry.BiLog : identity), OffsetResults = false, OffsetResultsBy = nothing)
    @assert length(idxs) == 2 && allunique(idxs) && all(1 .≤ idxs .≤ length(Points[1]))
    ...
    _, _, Res, ResPoints = _GetStochastic2DProfile(Points, Likelihoods, idxs; Extremizer, OffsetResults, OffsetResultsBy, nxbins, nybins, pxval, pyval, pxBins, pyBins)
    res = Trafo.(Res)
